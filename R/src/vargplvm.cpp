...
//    void getMatrix(SEXP Rvec, matrix * data);
   // SEXP covertMatrix(matrix *inmat);
    ...
    SEXP vargplvm(SEXP M_int, SEXP N_int, SEXP Q_int, SEXP A_vec,
                  ...
                  SEXP ZmZm_arr, SEXP covGrad_arr, SEXP filedir);
                  ...
                  SEXP ZmZm_arr, SEXP covGrad_arr,SEXP filedir)
    {
        ...
        PROTECT(filedir = AS_CHARACTER(filedir));
        pfiledir = R_alloc(strlen(CHAR(STRING_ELT(filedir, 0))), sizeof(char));
        strcpy(pfiledir, CHAR(STRING_ELT(filedir, 0)));
        UNPROTECT(1);
        //printf("pfile in cpp %s\n", pfiledir);
