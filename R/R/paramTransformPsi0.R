...
function (kern, gKern)
{
    ...
    if (length(kern$transforms) > 1)
    {
        for (i in 1:length(kern$transforms))
        {
            ...
            if (!(kern$transforms[[i]] == "transformsettings"))
        ...
        if (!("transformsettings" %in% names(kern$transforms[[1]])))
            gKern[kern$transforms[[1]]$index] <- gKern[kern$transforms[[1]]$index]*do.call(fhandle,list(params[kern$transforms[[1]]$index], "gradfact")) 
        else
