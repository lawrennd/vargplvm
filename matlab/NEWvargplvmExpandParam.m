...
%----------- TEMP
if isfield(model, 'tiedX')
    origX = model.X(:,model.tiedX);
    origMeans = model.vardist.means(:,tiedX);
    origCovars = model.vardist.covars(:,tiedX);
end
%--------
...
%----------- TEMP
if isfield(model, 'tiedX')
    model.X(:,model.tiedX) = origX;
    model.vardist.means(:,model.tiedX) = origMeans;
    model.vardist.covars(:,model.tiedX) = origCovars;
end
%--------
